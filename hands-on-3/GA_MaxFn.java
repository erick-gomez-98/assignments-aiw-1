...
class Chromosome {
    ...
    public void fitness(){
        int x = binaryToInteger(this.genes);
        this.fitness = (float) Math.pow(x, 2);
        this.xValue = x;
    }

    public int binaryToInteger(char[] binary){
        int result = 0;
...
class Population {
    ...
    public void isFinished(){
        for(int i = 0; i < population.length; i++){
            if(Arrays.equals(population[i].genes, target.toCharArray())){
...
public class GA_MaxFn {
    public static void main(String args[]){
        Population population = new Population(0.01f, 0.95f, "11110", 4);
